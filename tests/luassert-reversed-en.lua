...
--
-- This is a copy of luassert's en.lua, where the order of "Expected" and
-- "Passed" are reversed to suit my preference (which is roughly what every
-- other test library does... as it's better to pass first the variable, then
-- the literal result you expect).

-- luacheck: no max line length

local s = require('say')

-- An alternative way that I discarded, and can't remember why, but it's worth
-- remembering that it's there if a problem arises.
-- s:set_namespace('reversed-en')
